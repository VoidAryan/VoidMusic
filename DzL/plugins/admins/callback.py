...
from DzL.misc import SUDOERS, db
from DzL.utils.database import (is_active_chat,
...
from DzL.utils.formatters import seconds_to_min
from DzL.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from DzL.utils.stream.autoclear import auto_clean
from DzL.utils.thumbnails import gen_thumb
    ...
    if command == "Pause":
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Dz.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Dz.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Dz.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
